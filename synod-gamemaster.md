...
export class GameMaster {
  ...
  applyMove(move: CouncilMove): BuyerSignal {
    ...
    if (cooperative) {
      const willingness = s.revealed_need ? s.reservation : s.reservation; // ceiling is always R
      const step = CONCESSION_BASE * (s.trust / 100) * (s.revealed_need ? 1.6 : 1);
      s.current_ask = Math.min(willingness, Math.round(s.current_ask + step));
    }
    ...
    if (move.action === "accept") {
      // seller accepts the buyer's current ask
      this.finalPrice = s.current_ask;
      this.terminal = "accepted";
      return this.signal({ accepted: true });
    }
    if (sellerPrice !== undefined && sellerPrice <= s.current_ask && sellerPrice <= s.reservation) {
      this.finalPrice = sellerPrice; // buyer meets the seller at the seller's (lower-than-ask) price
      this.terminal = "accepted";
      return this.signal({ accepted: true });
    }
  ...
  private signal(flags: { walked?: boolean; accepted?: boolean }): BuyerSignal {
    ...
    this.history.push({ move: this.history.length ? this.history[this.history.length - 1].move : { action: "hold" }, signal: sig });
    return sig;
  }
}
...
const adaptive: Record<CounterpartyType, Policy> = {
  ...
  firm_floor: [
    ...
    { action: "counter_soft", offer: { price: 9400 } }, // recognize firm reality, price near R
    { action: "counter_soft", offer: { price: 9350 } },
  ],
  deceptive: [
